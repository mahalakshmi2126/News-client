...
const UserAuthenticationPage = () => {
  const { setUser, setIsAuthenticated, refreshUserData } =
  ...
  const redirectPath =
    searchParams.get("redirect") || "/personalized-news-dashboard";
...
const handleLogin = async (formData) => {
  ...
  const handleRegister = async (formData) => {
    ...
    try {
      ...
      navigate(
        `/user-authentication-login-register?redirect=${encodeURIComponent(
          redirectPath
        )}`
      );
    } catch (error) {
